...
export default function DelegatesPage() {
  ...
  const fetchDelegates = async () => {
    try {
      ...
      // ADDED: Get current delegation status if user is connected
      if (user?.walletAddress) {
        const status = await getDelegationStatus(user.walletAddress);
        // Mark the delegate that the user has delegated to
        if (status.isDelegating) {
          const updatedList = delegateList.map(delegate => ({
            ...delegate,
            isDelegatedByUser: delegate.address.toLowerCase() === status.delegatedTo.toLowerCase()
          }));
          setDelegates(updatedList);
        } else {
          setDelegates(delegateList.map(delegate => ({
            ...delegate,
            isDelegatedByUser: false
          })));
        }
        logger.debug(`Fetched ${delegateList.length} delegates with delegation status`);
      } else {
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {delegates.map((delegate) => (
            <div key={delegate.address} className={`bg-gray-900 rounded-lg p-5 shadow-md border ${delegate.isDelegatedByUser ? 'border-indigo-500' : 'border-gray-800'}`}>
              <div className="flex justify-between items-start mb-4">
                <div>
                  <h3 className="text-white font-semibold text-lg mb-1">
                    {delegate.name || shortenAddress(delegate.address)}
                    {delegate.isDelegatedByUser && (
                      <span className="ml-2 text-xs bg-indigo-900 text-indigo-300 py-1 px-2 rounded-full">
                        Your Delegate
                      </span>
                    )}
                  </h3>
                  <p className="text-gray-400 text-xs font-mono">{delegate.address}</p>
                </div>
              ...
              <div className="mb-4">
                <div className="flex justify-between text-sm mb-1">
                  ...
                  <span className="text-indigo-400 font-medium">{parseFloat(delegate.votingPower).toFixed(2)}</span>
                </div>
              </div>

              <button
                onClick={() => initiateDelegation(delegate)}
                disabled={delegate.isDelegatedByUser || user?.walletAddress === delegate.address || !user?.walletAddress}
              ...
              >
                {delegate.isDelegatedByUser 
                  ? 'Currently Delegated' 
                  : user?.walletAddress === delegate.address
                    ? 'Your Address'
                    : 'Delegate Votes'}
              </button>
            </div>
