...
export const updateVoteInFirebase = async (
  ...
  try {
    logger.debug(`Updating vote in Firebase for proposal ${proposalId}`);
    
    // Get the proposal document or create it if it doesn't exist
    const proposalRef = doc(db, 'proposals', proposalId.toString());
    const proposalDoc = await getDoc(proposalRef);
    
    if (!proposalDoc.exists()) {
      logger.debug(`Proposal ${proposalId} not found in Firebase, creating it`);
      
      // Create a basic proposal document if it doesn't exist
    ...
    const proposalData = proposalDoc.exists() ? proposalDoc.data() : {};
    ...
    const existingVoteIndex = votes.findIndex(
      (v: any) => v.voter.toLowerCase() === voterAddress.toLowerCase()
    );
    ...
    logger.debug(`Updated vote in Firebase for proposal ${proposalId}`);
  } catch (error) {
    logger.error(`Error updating vote in Firebase for proposal ${proposalId}:`, error);
    throw error;
  }
};

/**
 * Update proposal data in Firebase
 * @param proposalId The proposal ID
 * @param data The fields to update
 */
export const updateProposalInFirebase = async (
  proposalId: number,
  data: Record<string, any>
): Promise<void> => {
  try {
    logger.debug(`Updating proposal ${proposalId} in Firebase`);
    ...
    logger.debug(`Updated proposal ${proposalId} in Firebase`);
  } catch (error) {
    logger.error(`Error updating proposal ${proposalId} in Firebase:`, error);
    throw error;
  }
}; 