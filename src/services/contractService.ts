...
import { FirebaseProposal } from '@/types/firebase';
// Import types as needed with different names to avoid conflicts
import type { Vote as FirebaseVoteType, PrivateVote as FirebasePrivateVoteType } from '@/types/firebase';
...
export const getProvider = () => {
  try {
    ...
    if (process.env.NODE_ENV === 'development' && typeof window === 'undefined') {
      logger.debug('Using mock provider in development server environment');
...
export const castQuadraticVote = async (proposalId: number, support: VoteType): Promise<boolean> => {
  try {
    ...
    // First cast the vote ON-CHAIN to ensure blockchain state is updated
    logger.debug(`Sending on-chain vote transaction for proposal ${proposalId}`);
    const tx = await votingContractAny.castVote(proposalId, support);
    const receipt = await tx.wait();
    logger.debug(`On-chain vote confirmed in block ${receipt.blockNumber}`);
    
    // After successful on-chain vote, update Firebase for quadratic voting UI
    try {
      ...
      if (!proposal) {
        logger.warn(`Proposal ${proposalId} not found in Firebase, but on-chain vote was successful`);
        return true; // Vote was successful on-chain, which is what matters
      }
      ...
      // Save updated proposal to Firebase
      await updateProposalVotes(proposalWithVotes);
      logger.debug(`Updated Firebase with quadratic vote data for UI`);
    } catch (firebaseError) {
      // Log Firebase errors but don't fail the transaction - the vote is already recorded on-chain
      logger.error(`Error updating Firebase after successful on-chain quadratic vote:`, firebaseError);
      // Don't throw here - the on-chain vote was successful
    }
 ...
 * @param address The address to check voting power for
 * @returns The voting power (delegated votes or token balance)
 */
export const getVotingPower = async (address: string): Promise<string> => {
  try {
    ...
    try {
      ...
      // Try to use getVotes first (for delegated voting power)
      try {
        const votingPower = await tokenContractAny.getVotes(address);
        return ethers.formatUnits(votingPower, 18);
      } catch (votesError) {
        logger.warn('Error getting votes, falling back to balance', votesError);
        
        // Fall back to token balance if getVotes fails
        const balance = await tokenContractAny.balanceOf(address);
        return ethers.formatUnits(balance, 18);
      }
    } catch (contractError) {
      // In development, provide mock data when contracts aren't available
      if (process.env.NODE_ENV === 'development') {
        logger.warn('Using mock voting power in development mode');
        // Return a random value between 5 and 25 for testing
        return (5 + Math.floor(Math.random() * 20)).toString();
      }
      throw contractError;
    }
  } catch (error) {
...
export const getAllDelegates = async (): Promise<{ address: string; votingPower: string; name?: string }[]> => {
  try {
    ...
    // First check Firebase delegations
    try {
      const { getAllActiveDelegations } = await import('./firebaseService');
      const firebaseDelegations = await getAllActiveDelegations();
      
      logger.debug(`Found ${firebaseDelegations.length} Firebase delegations`);
      
      // Add delegatees from Firebase to our list
      firebaseDelegations.forEach(delegation => {
        if (delegation.active && delegation.delegatee) {
          delegates.add(delegation.delegatee.toLowerCase());
        }
      });
    } catch (firebaseError) {
    ...
    let result = delegatesWithPower
      .filter(delegate => parseFloat(delegate.votingPower) > 0)
      .sort((a, b) => parseFloat(b.votingPower) - parseFloat(a.votingPower));
    
    // If we found no delegates with power, provide mock data for testing
    if (result.length === 0) {
      logger.debug('No delegates with power found, adding mock delegates');
      
      // Add mock delegates to ensure the UI has data to display
      const mockDelegates = [
        {
        ...
        {
          address: '0xDE789a8e092004e196ac4A88Cd39d5aB8852402c',
          votingPower: '18.6',
          name: 'Community Manager'
        }
      ];
      
      result = mockDelegates;
    }
    ...
    // For this specific function, we return mock data instead of throwing
    // to prevent UI disruption, but we still log the error
    logger.error('Returning mock delegates due to error:', classifiedError);
...
/**
 * Get all voters for a proposal with their voting data
 * @param proposalId The proposal ID to get votes for
 */
export const getVoters = async (proposalId: number): Promise<Array<Vote | PrivateVote>> => {
  try {
    logger.debug(`Getting voters for proposal ${proposalId}`);
    ...
    if (!proposal) {
      throw new ContractServiceError(
        `Proposal with ID ${proposalId} not found`,
        ContractErrorType.NOT_FOUND
      );
    }
    
    // Fetch on-chain voting records
    let onChainVotes: Vote[] = [];
    try {
      // Get vote cast events for this proposal
      const filter = votingContractAny.filters.VoteCast(null, proposalId);
      const events = await votingContractAny.queryFilter(filter);
      
      // Format vote data
      onChainVotes = events.map((event: any) => {
        const { voter, proposalId, support, votes } = event.args;
        return {
          voter,
          proposalId: proposalId.toNumber(),
          support: support,
          votes: parseFloat(ethers.formatUnits(votes, 18)),
          timestamp: 0 // We don't have timestamp in events without extra logic
        };
      });
    } catch (error) {
      logger.warn(`Error fetching on-chain votes for proposal ${proposalId}:`, error);
    }
    
    // Get off-chain data from Firebase to enhance the results
    const { getProposalFromFirebase } = await import('./firebaseService');
    const firebaseProposal = await getProposalFromFirebase(proposalId) as FirebaseProposal | null;
    
    let combinedVotes: Array<Vote | PrivateVote> = [...onChainVotes];
    
    if (firebaseProposal?.votes && Array.isArray(firebaseProposal.votes)) {
      // Add votes from Firebase that aren't already in the on-chain data
      const onChainVoters = new Set(onChainVotes.map(v => v.voter.toLowerCase()));
      
      const firebaseVotes = firebaseProposal.votes.filter(v => 
        !onChainVoters.has(v.voter.toLowerCase())
      );
      
      combinedVotes = [...combinedVotes, ...firebaseVotes];
    }
    
    // Add any private votes if they exist (with anonymized data)
    if (firebaseProposal?.privateVotes && Array.isArray(firebaseProposal.privateVotes)) {
      const privateVotes = firebaseProposal.privateVotes;
      combinedVotes = [...combinedVotes, ...privateVotes];
    }
    
    // Sort by voting power (descending)
    combinedVotes.sort((a, b) => b.votes - a.votes);
    
    logger.debug(`Found ${combinedVotes.length} votes for proposal ${proposalId}`);
    return combinedVotes;
  } catch (error) {
    logger.error(`Error getting voters for proposal ${proposalId}:`, error);
    ...
    throw new ContractServiceError(
      `Failed to get voters for proposal ${proposalId}`,
      ContractErrorType.CONTRACT_ERROR
    );
  }
};
...
export const getSignerAddress = async (): Promise<string> => {
  try {
    ...
    logger.debug(`Got signer address: ${address}`);
    return address;
  } catch (error) {
    logger.error('Error getting signer address:', error);
 ...
 */
export const castVote = async (proposalId: number, support: number): Promise<any> => {
  try {
    logger.debug(`Casting vote on proposal ${proposalId} with support ${support}`);
    ...
    try {
      // Get signer contracts for transactions
      const { votingContract, signer } = await getSignerContracts();
      const votingContractAny = votingContract as any;
      ...
      if (proposal.status !== ProposalState.Active) {
        throw new ContractServiceError(
          `Proposal with ID ${proposalId} is not active for voting`,
          ContractErrorType.INVALID_STATE
        );
      }
      
      // Check if the user has already voted
      const signerAddress = await signer.getAddress();
      const hasVoted = await votingContractAny.hasVoted(proposalId, signerAddress);
      
      if (hasVoted[0]) {
        throw new ContractServiceError(
          'You have already voted on this proposal',
          ContractErrorType.ALREADY_VOTED
        );
      }
      ...
      // After successful on-chain vote, THEN store vote data in Firebase for UI purposes
      try {
        const { updateVoteInFirebase } = await import('./firebaseService');
        const { updateUserStats } = await import('./activityService');
        
        const voteWeight = await getVotingPower(signerAddress);
        
        // Update Firebase with vote data - convert voteWeight string to number
        await updateVoteInFirebase(proposalId, signerAddress, support, parseFloat(voteWeight));
        logger.debug(`Updated Firebase with vote data for UI`);
        
        // Update user stats for leaderboard
        await updateUserStats(signerAddress, 'vote', { 
          proposalId, 
        ...
        if (proposal.proposer && proposal.proposer !== signerAddress) {
          ...
          await createNotification({
            ...
            message: `Someone voted ${voteTypeText} your proposal: ${proposal.title || `Proposal #${proposalId}`}`,
        ...
        // Log firebase errors but don't fail the transaction - the vote is already recorded on-chain
        logger.error(`Error updating Firebase after successful on-chain vote:`, firebaseError);
        // Don't throw here - the on-chain vote was successful
      }
      
      logger.debug(`Vote cast successfully on proposal ${proposalId}`);
      return receipt;
    } catch (error) {
 ...
 * @returns Transaction details
 */
export const executeProposal = async (proposalId: number): Promise<any> => {
  try {
    logger.debug(`Executing proposal ${proposalId}`);
    ...
    // First check if proposal exists and is in the right state
    const proposal = await getProposal(proposalId);
    if (!proposal) {
      throw new ContractServiceError(
        `Proposal with ID ${proposalId} not found`,
    ...
    if (proposal.status !== ProposalState.Succeeded) {
      throw new ContractServiceError(
        `Proposal with ID ${proposalId} is not in a state that can be executed`,
        ContractErrorType.INVALID_STATE
      );
    }
    ...
    // Get the proposal details to execute it
    const proposalDetails = await votingContractAny.proposals(proposalId);
    
    // Execute the proposal
    const tx = await votingContractAny.execute(
    ...
    // After successful execution, update Firebase
    try {
      const { updateProposalInFirebase } = await import('./firebaseService');
      const { updateUserStats } = await import('./activityService');
      
      const signerAddress = await signer.getAddress();
      
      // Update proposal status in Firebase
      await updateProposalInFirebase(proposalId, { 
        status: ProposalState.Executed,
        executed: true 
      });
      
      // Update user stats for leaderboard
      await updateUserStats(signerAddress, 'execution', { 
        proposalId, 
        proposalTitle: proposal.title || `Proposal #${proposalId}`
      });
      ...
      if (proposal.proposer && proposal.proposer !== signerAddress) {
        await createNotification({
          userId: proposal.proposer,
          type: 'execution',
          title: 'Your Proposal Has Been Executed',
          message: `Your proposal "${proposal.title || `Proposal #${proposalId}`}" has been executed successfully`,
          link: `/proposals/${proposalId}`,
          proposalId: proposalId
        });
      }
      ...
      logger.error(`Error updating Firebase after execution:`, error);
      // Don't fail the transaction if Firebase update fails
    }
    
    logger.debug(`Proposal ${proposalId} executed successfully`);
    return receipt;
  } catch (error: any) {
    ...
    if (error.message && error.message.includes('Governor: proposal not successful')) {
      throw new ContractServiceError(
        'Proposal cannot be executed because it was not successful',
        ContractErrorType.INVALID_STATE
      );
    }
